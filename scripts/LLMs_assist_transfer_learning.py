...
def seq_cluster(text_lists,thred=0.75):
    ...
    for i in range(len(text_lists)):
        ...
        for cluster in clusters:
            for id in cluster:
                score=pairwise2.align.localxx(Seq(text_lists[i]),Seq(text_lists[id]))[0][2]/max(len(text_lists[i]),len(text_lists[id]))
...
def blast_similirity(train_seq,test_seq):
    ...
    for i in range(len(test_seq)):
        ...
        for j in range(len(train_seq)):
            score=pairwise2.align.localxx(Seq(test_seq[i]),Seq(train_seq[j]))[0][2]/max(len(test_seq[i]),len(train_seq[j]))
...
class Model(nn.Module):
    def __init__(self,vocab_size,embedding_dim,pred_dim,dropout,nhead,num_encoder_layers):
        ...
        encoder_layer_seq_dnapt2 = nn.TransformerEncoderLayer(d_model=embedding_dim , nhead=nhead, dropout=dropout,batch_first=True)
        self.transformer_encoder_seq_dnapt2 = nn.TransformerEncoder(encoder_layer_seq_dnapt2, num_layers=num_encoder_layers)
...
def train_fold():
    ...
    testStruOri_130 = np.array([testDataFrame_130['structure'].values.tolist()[max_simi_id],testDataFrame_130['structure'].values.tolist()[min_simi_id]])
    testLoopOri_130 = np.array([testDataFrame_130['predicted_loop_type'].values.tolist()[max_simi_id],testDataFrame_130['predicted_loop_type'].values.tolist()[min_simi_id]])
    ...
    testData_labels_130 = np.array([testDataFrame_130[pred_cols_test].values.tolist()[max_simi_id],testDataFrame_130[pred_cols_test].values.tolist()[min_simi_id]])
    ...
    dataset_test_130_nopretrain = RNADataset_nopretrain(testSeq_130, testSeqOri_130,testStru_130,testLoop_130,testData_labels_num_130,As_test_130)
...
def tSNE():
    ...
    embedding_all_seq_reshape=np.reshape(embedding_all_seq, (len(embedding_all_seq), len(embedding_all_seq[0])*len(embedding_all_seq[0][0])))
