...
def MCRMSE_NAN_SW_TEW(y_pred, y_true, sam_weig=None, tew=None):
    ...
    s = (mean_squared_sw_tew(y_true[:, :, 0], y_pred[:, :, 0], sample_weight=sam_weig, exp_err=tew[:, :, 0]) / 1.0) * \
        LOSS_WGTS[0]

    for i in range(1, len(pred_cols_train)):
        s += (mean_squared_sw_tew(y_true[:, :, i], y_pred[:, :, i], sample_weight=sam_weig,
                                  exp_err=tew[:, :, i]) / 1.0) * LOSS_WGTS[i]
    return s
...
def train_fold():
    ...
    testDataFrame_pub = pd.read_json('../data/test_dataset_public.json')
    ...
    train_exp_errors = np.transpose(np.array(trainDataFrame[pred_cols_errors].values.tolist(), dtype='float32'),
                                    (0, 2, 1))
    train_error_weights = error_alpha + np.exp(-train_exp_errors * error_beta)
    ...
    trainSeqOri_aug = np.array(trainSeqOri + trainSeqOri_rev)
    ...
    trainStruOri_aug = np.array(trainStruOri + trainStruOri_rev)
    ...
    trainLoopOri_aug = np.array(trainLoopOri + trainLoopOri_rev)
    ...
    print('trainStruAdj shape:', trainStruAdj_aug.shape)
    ...
    print('train_bp_matrix shape:', train_bp_matrix_aug.shape)
    ...
    As_train_aug = np.concatenate([As_train_aug[:, :, :, None], trainStruAdj_aug, Ds_train_aug, DDs_train_aug],
                                  axis=3).astype(np.float32)
    As_test_pub = np.concatenate([As_test_pub[:, :, :, None], testStruAdj_pub, Ds_test_pub, DDs_test_pub],
                                 axis=3).astype(np.float32)
    ...
    testData_labels_num_pub = np.array(testDataFrame_pub[pred_cols_test].values.tolist(), dtype='float32').transpose(
        (0, 2, 1))
    ...
    dataset_train = RNADataset(trainSeq_aug, trainSeqOri_aug, trainStru_aug, trainLoop_aug, train_labels_aug,
                               As_train_aug, train_sam_wei_aug, train_error_weights_aug, sam_aug_flag)
    dataset_test_pub = RNADataset(testSeq_pub, testSeqOri_pub, testStru_pub, testLoop_pub, testData_labels_num_pub,
                                  As_test_pub)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(dataloader_train, 0):
            ...
            loss_pre = criterion(pre_out[samaugflag == 0, :train_seq_scored], labels[samaugflag == 0],
                                 sam_weig[samaugflag == 0], tew[samaugflag == 0]) + criterion(
                pre_out[samaugflag == 1, (train_seq_len - train_seq_scored):], labels[samaugflag == 1],
                sam_weig[samaugflag == 1],
                tew[samaugflag == 1])
        ...
        for batch_idx, data in enumerate(dataloader_test_pub, 0):
            ...
            loss = metric_func(pre_out[:, :test_seq_scored_pub, :pred_dim_pub], labels)
        ...
        for batch_idx, data in enumerate(dataloader_test_107, 0):
            ...
            loss = metric_func(pre_out[:, :test_seq_scored_107, :pred_dim_107], labels)
        ...
        for batch_idx, data in enumerate(dataloader_test_130, 0):
            ...
            loss = metric_func(pre_out[:, :test_seq_scored_130, :pred_dim_130], labels)
        ...
        scheduler.step()
        print("epoch: [%d], train loss: [%.6f], public loss: [%.6f], private loss: [%.6f]" % (
            epoch, train_pre_loss / train_batch_num, test_loss_pub / test_batch_num_pub,
            ((test_loss_107 / test_batch_num_107) + (test_loss_130 / test_batch_num_130)) / 2.0))
    ...
    torch.save(best_model_weights, "./best_model_weights")
