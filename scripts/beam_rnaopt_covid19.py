...
def get_new_seq_global(seq_ori_charlist, id, mimazi_opt):
    import copy
    ...
    return ''.join(seq_ori_charlist_temp)

def process_variation_global(args, seq_ori_charlist, sorted_id, seq_ori_mfe):
    i, temp_mimazi = args
    new_seq = get_new_seq_global(seq_ori_charlist, sorted_id[i], temp_mimazi)
    temp_mfe = free_energy(new_seq, package='vienna_2')  # Assuming free_energy is a globally accessible function
    return (new_seq, temp_mfe)
def seq_opti_parallel(seq_ori, sorted_id, seq_ori_mfe):
    ...
    for i in range(len(mimazi_ori)):
        for temp_mimazi in mimazi_can_choice[i]:
            if temp_mimazi!=mimazi_ori[i]:
                tasks.append((i, temp_mimazi))

    with Pool() as pool:
        results = pool.starmap(process_variation_global, [(task, seq_ori_charlist, sorted_id, seq_ori_mfe) for task in tasks])

    seq_after_tubian = []
    seq_after_tubian_mfe = []
    for new_seq, temp_mfe in results:
        if temp_mfe < seq_ori_mfe:
            seq_after_tubian.append(new_seq)
            seq_after_tubian_mfe.append(temp_mfe)
    if len(seq_after_tubian)==0:
    ...
    final_seq = [seq_after_tubian[i] for i in sorted_mfe]
    final_score = [seq_after_tubian_mfe[i] for i in sorted_mfe]

    return final_seq, final_score

def beam_search_single(seq_ori,len_seq_fenge,seq_ori_mfe,num_per_seq_tansuo):
    ...
    for i in range(seq_ori_len//len_seq_fenge + 1):
        ...
        model.load_state_dict(torch.load('./best_model_weights_withoutstrloopdnabert', map_location='cpu'))
        model.eval()
    ...
    vale,sorted_id=max_deg_pro(pre_out_ph10,num_per_seq_tansuo)
    mimazi=max_deg_mimazi(seq_ori,sorted_id)
    final_seq,final_score=seq_opti_parallel(seq_ori,sorted_id,seq_ori_mfe)
...
def beam_search_multi(seq_list,score_list):
    ...
    for i in range(min(len(seq_list),seq_sear_num)):
        final_seq_single,final_score_single=beam_search_single(seq_list[i],999,score_list[i],beam_sear_geshu//min(len(seq_list),seq_sear_num)+mimazi_sear_num_bias)
        seq_multi.extend(final_seq_single)
        score_multi.extend(final_score_single)

    unique_seqs={}
    for i in range(len(seq_multi)):
        if seq_multi[i] not in unique_seqs:
            unique_seqs[seq_multi[i]]=score_multi[i]
    seq_multi = list(unique_seqs.keys())
    score_multi = list(unique_seqs.values())
...
seq_ori_ge_WT=[""]#seq len 1869       
#seq_ori_ge_WT=["AUGGGGACAGUUAAUAAACCAUCGGGGUUCAAAUCGGGCAUGGGGACAGUUAAUAAACCAUCGGGGUUCAAAUCGGGCAUGGGGACAGUUAAUAAACCAUCGGGGUUCAAAUCGGGCAUGGGGACAGUUAAUAAACCAUCGGGGUUCAAAUCGGGC"]

best_mfe=free_energy(seq_ori_ge_WT[0], package='vienna_2')
best_seq=seq_ori_ge_WT[0]
print('seq_ori_mfe:')
print(best_mfe)
score_ori_list=[best_mfe]

final_seq_multi,final_score_multi=beam_search_multi(seq_ori_ge_WT,score_ori_list)
